...
| Creation and conversion controls | Ready locally for DD-03 boundary behavior: native bitmap creation is observable, but there is no v1 convert-back command. The remaining public command-surface cleanup is tracked by DD-01/DD-07. | Implemented with draft `BITMAP CONVERT` still present in current `unstable`. `no` preserves legacy string creation unless conversion is explicit; `yes` creates/converts native bitmaps on bitmap writes. | [#20](https://github.com/aviggiano/redis/issues/20), [#22](https://github.com/aviggiano/redis/issues/22), [#26](https://github.com/aviggiano/redis/issues/26), [PR #13](https://github.com/aviggiano/redis/pull/13), [PR #15](https://github.com/aviggiano/redis/pull/15) |
...
| Generic string command boundary | Ready locally: bitmap commands support both legacy strings and native bitmaps; generic string commands reject native bitmap values with `WRONGTYPE`; v1 has no explicit native-to-string materialization command. | Implemented with regression coverage for generic string commands, string-adjacent reads, Lua, modules, and legacy string bitmap behavior. Plain `SET` can still replace any key with a string. | [#22](https://github.com/aviggiano/redis/issues/22), [PR #8](https://github.com/aviggiano/redis/pull/8), [PR #39](https://github.com/aviggiano/redis/pull/39) |
...
| `R.SETBITARRAY`, `R.GETBITARRAY`, and `R64.*` equivalents | Use ASCII `0`/`1` bit-array strings rather than Redis raw bitmap strings. | Migration-tool-only | Optional compatibility format for tooling. Import can translate ASCII bit arrays to set offsets or target-native RESTORE payloads; Redis core v1 should keep compatibility in bitmap commands rather than adding string materialization commands. |
...
- A bitmap write that creates or converts a native bitmap because of
  ...
  triggering command.
- The current draft `BITMAP CONVERT` queues the same RESTORE form, preserving
  TTL with `ABSTTL` when needed; the v1 command surface is still being cleaned
  up under DD-01/DD-07.
...
The current `unstable` draft surface is a single `bitmap-default-roaring
yes|no` flag, defaulting to `no`, plus draft `BITMAP CONVERT` support. DD-03 no
longer depends on the explicit conversion command, and v1 should not expose a
native-to-string materialization command.
...
- The current draft `BITMAP CONVERT <key> STRING` path is not part of DD-03's
  final v1 boundary.
...
- **Conversion/public surface**: confirm whether `BITMAP CONVERT` is needed in
  v1 and whether it is the only new user-facing bitmap command. DD-03 already
  settles that v1 has no explicit native-to-string materialization command. See
  [#20](https://github.com/aviggiano/redis/issues/20) and
  [#26](https://github.com/aviggiano/redis/issues/26).
