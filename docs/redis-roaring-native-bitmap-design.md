...
> `bitmap-roaring`, `bitmap-default-roaring`, direct native bitmap command
> support, and RESTORE-based type-transition propagation. DD-17
...
- Use a native `OBJ_BITMAP` type for values that are created through the
  configured native-bitmap path or loaded from native bitmap payloads.
...
- Bitmap-command writes may transition string bitmap values to `bitmap` when
  native creation is enabled, currently through `bitmap-default-roaring yes` or
  native load/RESTORE/replication paths.
- Generic string commands reject native bitmap values with `WRONGTYPE`; v1 does
  not include an explicit native-to-string materialization command.
...
| Creation and conversion controls | Ready locally: v1 keeps legacy string bitmap creation by default, omits threshold configs, and does not expose `BITMAP CONVERT`. | Implemented. `bitmap-default-roaring no` preserves legacy string creation unless the key is already native through load/RESTORE/replication or a native BITOP source; `yes` creates/converts native bitmaps on bitmap writes. | [#20](https://github.com/aviggiano/redis/issues/20), [#22](https://github.com/aviggiano/redis/issues/22), [#26](https://github.com/aviggiano/redis/issues/26), [PR #13](https://github.com/aviggiano/redis/pull/13), [PR #15](https://github.com/aviggiano/redis/pull/15) |
...
| Generic string command boundary | Ready locally: bitmap commands support both legacy strings and native bitmaps; generic string commands reject native bitmap values with `WRONGTYPE`; ordinary writes such as `SET` can replace a native key with a string; v1 has no explicit native-to-string materialization command. | Implemented with regression coverage for generic string commands, string-adjacent reads, Lua, modules, and legacy string bitmap behavior. | [#22](https://github.com/aviggiano/redis/issues/22), [PR #8](https://github.com/aviggiano/redis/pull/8), [PR #39](https://github.com/aviggiano/redis/pull/39) |
| Persistence, `DUMP` / `RESTORE`, and AOF rewrite | Settled for the current direction: persist Redis-owned observable bitmap bytes rather than an opaque CRoaring portable blob, and keep RESTORE-based rewrite for native bitmap values. | Implemented with `RDB_TYPE_BITMAP`, `byte_len` plus a Redis raw string payload, `DUMP` / `RESTORE`, AOF rewrite as `RESTORE`, and always-on payload validation. The PR has not shipped, so loaders do not preserve compatibility for earlier in-PR draft raw/range/container/portable bitmap payloads. See `docs/redis-roaring-native-bitmap-rdb-format.md`. | [#29](https://github.com/aviggiano/redis/issues/29), [PR #4](https://github.com/aviggiano/redis/pull/4), [PR #43](https://github.com/aviggiano/redis/pull/43), related open [PR #44](https://github.com/aviggiano/redis/pull/44) |
...
| redis-roaring migration contract | Ready locally: v1 uses an external streaming migrator with fail-closed validation, a durable manifest, explicit cap policy, and no Redis core legacy payload loader or `R.*` / `R64.*` compatibility surface. Offline RDB transcoding is deferred as a possible future expert mode. | Redis core keeps the contract in `docs/redis-roaring-migration-contract.md`; the reference tool lives in `aviggiano/redis-roaring` via [aviggiano/redis-roaring#148](https://github.com/aviggiano/redis-roaring/pull/148). No Redis core command or RDB-loader change is required for this decision. | [#34](https://github.com/aviggiano/redis/issues/34) |
...
| `R.SETBITARRAY`, `R.GETBITARRAY`, and `R64.*` equivalents | Use ASCII `0`/`1` bit-array strings rather than Redis raw bitmap strings. | Migration-tool-only | Optional compatibility format for tooling. Import can translate ASCII bit arrays to set offsets or target-native RESTORE payloads; Redis core should keep compatibility in bitmap commands rather than adding string materialization commands. |
...
`docs/redis-roaring-migration-contract.md`. The reference implementation lives
in [`aviggiano/redis-roaring`](https://github.com/aviggiano/redis-roaring) as
...
exposes native creation through `bitmap-default-roaring yes`, while keeping
threshold configs and public conversion commands out of v1.
...
- Version-skew policy is upgrade-first. Replicas and AOF replay targets must
  ...
  bitmap creation on a master.

## Creation and Conversion Surface

The v1 surface is a single `bitmap-default-roaring yes|no` flag, defaulting to
`no`. Threshold-based selection configs and public conversion commands are out
of scope for v1.

- With `bitmap-default-roaring no`, bitmap writes keep creating strings unless
  the key is already native through load/restore/replication or a native BITOP
  source.
...
This settles the command/config shape tracked in
...
- **String/bitmap command boundary**: native bitmaps remain an observable
  `bitmap` type. Redis bitmap commands support both legacy string bitmap values
  and native bitmap values, while generic string commands reject native bitmap
  values with `WRONGTYPE`. V1 has no explicit native-to-string materialization
  command; ordinary writes such as `SET` can still replace a key with a string.
  See [#22](https://github.com/aviggiano/redis/issues/22).
...
- **Creation controls**: v1 preserves legacy string bitmap creation by default,
  keeps threshold-based conversion configs out, and does not expose
  `BITMAP CONVERT`. Native creation is controlled by `bitmap-default-roaring`
  and by load/RESTORE/replication paths. See
  [#26](https://github.com/aviggiano/redis/issues/26).

## Pending Decisions
- **Persistence format and AOF rewrite**: confirm whether the current
  `RDB_TYPE_BITMAP` raw bitmap payload layout and RESTORE-based AOF rewrite are
  the final upstream shape for this core type. The current implementation stores
  Redis-owned observable bitmap bytes and does not preserve loaders for
  unreleased in-PR draft raw/range/container/portable payloads; open
