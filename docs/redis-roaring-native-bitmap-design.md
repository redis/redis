...
| Persistence, `DUMP` / `RESTORE`, and AOF rewrite | Pending clarification for the final RDB payload and whether RESTORE-based rewrite remains acceptable for this core type. | Implemented in the current draft with `RDB_TYPE_BITMAP`, encoding-specific raw or range payloads, `DUMP` / `RESTORE`, and AOF rewrite as `RESTORE`. | [#29](https://github.com/aviggiano/redis/issues/29), [PR #4](https://github.com/aviggiano/redis/pull/4), [PR #43](https://github.com/aviggiano/redis/pull/43), related open [PR #44](https://github.com/aviggiano/redis/pull/44) |
...
- **Persistence format and AOF rewrite**: confirm the final `RDB_TYPE_BITMAP`
  ...
  stores encoding-specific raw or range payloads; open
