...
int evictionPoolPopulate(redisDb *db, kvstore *samplekvs, struct evictionPoolEntry *pool) {
    ...
    for (j = 0; j < count; j++) {
        ...
        if (server.maxmemory_policy & MAXMEMORY_FLAG_LRU || server.maxmemory_policy & MAXMEMORY_FLAG_LRP) {
...
int performEvictions(void) {
    ...
    while (mem_freed < (long long)mem_tofree) {
        ...
        if (server.maxmemory_policy & (MAXMEMORY_FLAG_LRU|MAXMEMORY_FLAG_LFU|MAXMEMORY_FLAG_LRP) ||
