...
#define BITMAP_RDB_CONTAINER_BITSET 1
#define BITMAP_RDB_CONTAINER_ARRAY 2
#define BITMAP_RDB_CONTAINER_RUN 3

static int bitmapContainerTypeToRdb(uint8_t typecode, uint64_t *rdb_typecode) {
    switch (typecode) {
    case BITSET_CONTAINER_TYPE:
        *rdb_typecode = BITMAP_RDB_CONTAINER_BITSET;
        return C_OK;
    case ARRAY_CONTAINER_TYPE:
        *rdb_typecode = BITMAP_RDB_CONTAINER_ARRAY;
        return C_OK;
    case RUN_CONTAINER_TYPE:
        *rdb_typecode = BITMAP_RDB_CONTAINER_RUN;
        return C_OK;
    default:
        return C_ERR;
    }
...
static int bitmapContainerTypeFromRdb(uint64_t rdb_typecode, uint8_t *typecode) {
    switch (rdb_typecode) {
    case BITMAP_RDB_CONTAINER_BITSET:
        *typecode = BITSET_CONTAINER_TYPE;
        return C_OK;
    case BITMAP_RDB_CONTAINER_ARRAY:
        *typecode = ARRAY_CONTAINER_TYPE;
        return C_OK;
    case BITMAP_RDB_CONTAINER_RUN:
...
static uint16_t bitmapRdbRead16(const char *p, int little) {
    uint16_t v;
    memcpy(&v, p, sizeof(v));
    if (little) memrev16ifbe(&v);
    return v;
...
static void bitmapObjectRdbContainer(const roaring64_bitmap_t *roaring,
                                     roaring64_leaf_t leaf,
                                     const container_t **container,
                                     uint8_t *typecode)
{
    *typecode = roaring64_leaf_typecode(leaf);
    *container = roaring->containers[roaring64_leaf_index(leaf)];
...
static int bitmapContainerMaxPayloadLen(uint8_t typecode, uint64_t cardinality,
                                        size_t *max_payload_len)
{
    ...
    switch (typecode) {
    case ARRAY_CONTAINER_TYPE:
        if (cardinality > DEFAULT_MAX_SIZE) return C_ERR;
        *max_payload_len = (size_t)cardinality * sizeof(uint16_t);
        return C_OK;
    case BITSET_CONTAINER_TYPE:
        if (cardinality <= DEFAULT_MAX_SIZE) return C_ERR;
        *max_payload_len = BITSET_CONTAINER_SIZE_IN_WORDS * sizeof(uint64_t);
        return C_OK;
    case RUN_CONTAINER_TYPE:
        *max_payload_len = sizeof(uint16_t) +
            (size_t)BITMAP_RDB_MAX_CONTAINER_CARDINALITY * sizeof(rle16_t);
        return C_OK;
    default:
        return C_ERR;
    }
...
{
    size_t max_payload_len;
    if (bitmapContainerMaxPayloadLen(typecode, cardinality,
                                     &max_payload_len) != C_OK)
        return C_ERR;
    if (payload_len > max_payload_len) return C_ERR;
...
static int bitmapContainerConvertEndian(char *payload, size_t payload_len,
                                        uint8_t typecode, int from_little)
{
#if BYTE_ORDER == BIG_ENDIAN
    switch (typecode) {
    case ARRAY_CONTAINER_TYPE:
    ...
    case RUN_CONTAINER_TYPE: {
        if (payload_len < sizeof(uint16_t)) return C_ERR;
        uint16_t runs = bitmapRdbRead16(payload, from_little);
...
static sds bitmapLoadRdbIntegerPayload(rio *rdb, int enctype,
                                       size_t max_payload_len,
                                       size_t *payload_len)
{
    unsigned char enc[4];
    ...
    if (len < 0 || (size_t)len > max_payload_len) return NULL;
    if (payload_len) *payload_len = (size_t)len;
    return sdsnewlen(buf, (size_t)len);
}

static sds bitmapLoadRdbLzfPayload(rio *rdb, size_t max_payload_len,
                                   size_t *payload_len)
{
    uint64_t clen = rdbLoadLen(rdb, NULL);
    uint64_t len = rdbLoadLen(rdb, NULL);
    if (clen == RDB_LENERR || len == RDB_LENERR) return NULL;
    if (len > max_payload_len || clen == 0 || clen > len) return NULL;

    unsigned char *compressed = ztrymalloc((size_t)clen);
    sds payload = sdstrynewlen(SDS_NOINIT, (size_t)len);
    if (compressed == NULL || payload == NULL) {
        zfree(compressed);
        sdsfree(payload);
        return NULL;
    }

    if (rioRead(rdb, compressed, (size_t)clen) == 0 ||
        lzf_decompress(compressed, (size_t)clen, payload, (size_t)len) != len) {
        zfree(compressed);
        sdsfree(payload);
        return NULL;
    }
...
static sds bitmapLoadRdbContainerPayload(rio *rdb, size_t max_payload_len,
                                         size_t *payload_len)
{
    int isencoded;
    ...
    if (isencoded) {
        ...
        case RDB_ENC_INT16:
        case RDB_ENC_INT32:
            return bitmapLoadRdbIntegerPayload(rdb, (int)len, max_payload_len,
                                               payload_len);
        case RDB_ENC_LZF:
            return bitmapLoadRdbLzfPayload(rdb, max_payload_len, payload_len);
        default:
            return NULL;
        }
    }

    if (len > max_payload_len) return NULL;
    sds payload = sdstrynewlen(SDS_NOINIT, (size_t)len);
    if (payload == NULL) return NULL;
    if (payload_len) *payload_len = (size_t)len;
    if (len && rioRead(rdb, payload, (size_t)len) == 0) {
        sdsfree(payload);
        return NULL;
    }
    return payload;
...
ssize_t bitmapObjectSaveRdbContainers(rio *rdb, const robj *o) {
    ...
    while (it.value != NULL) {
        ...
        const container_t *container;
        uint8_t typecode;
        bitmapObjectRdbContainer(roaring, leaf, &container, &typecode);

        uint64_t high48 = bitmapArtKeyToHigh48(it.key);
        uint64_t rdb_typecode;
        ...
        if (bitmapContainerTypeToRdb(typecode, &rdb_typecode) != C_OK)
            return -1;

        serverAssert(high48 <= BITMAP_RDB_MAX_HIGH48);
        serverAssert(cardinality > 0);
        serverAssert(payload_len >= 0);

        if ((n = rdbSaveLen(rdb, high48)) == -1) return -1;
        nwritten += n;
        if ((n = rdbSaveLen(rdb, rdb_typecode)) == -1) return -1;
        nwritten += n;
...
robj *createBitmapObjectFromRdbContainers(rio *rdb, uint64_t byte_len) {
    ...
    uint64_t container_count = rdbLoadLen(rdb, NULL);
    if (container_count == RDB_LENERR) return NULL;
    ...
    for (uint64_t i = 0; i < container_count; i++) {
        uint64_t high48 = rdbLoadLen(rdb, NULL);
        uint64_t raw_typecode = rdbLoadLen(rdb, NULL);
        uint64_t cardinality = rdbLoadLen(rdb, NULL);
        if (high48 == RDB_LENERR ||
            raw_typecode == RDB_LENERR ||
            cardinality == RDB_LENERR) goto fail;
        ...
        uint8_t typecode;
        if (bitmapContainerTypeFromRdb(raw_typecode, &typecode) != C_OK)
            goto fail;
        ...
        container_t *container = bitmapContainerFromRdbPayload(
            typecode, cardinality, payload, payload_len);
        sdsfree(payload);
        if (container == NULL) goto fail;

        bitmapObjectRoaringAppendContainer(roaring, high48, container, typecode);
    }

    if (!roaring64_bitmap_is_empty(roaring)) {
        uint64_t max = roaring64_bitmap_maximum(roaring);
        if (byte_len == 0 || max >= byte_len * 8) goto fail;
    }
    roaring64_bitmap_shrink_to_fit(roaring);
