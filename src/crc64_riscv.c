 ...
 *
 * Called only after baseline code has checked for Zbc support.
 *
...
static uint64_t crc64_riscv_zbc_impl(uint64_t crc, const uint8_t *buf, size_t len){
...
extern uint64_t crc64_riscv_zbc(uint64_t crc, const unsigned char *buf, uint64_t len) {
#if defined(__riscv_xlen) && (__riscv_xlen == 64) && defined(__riscv_zbc) && (__riscv_zbc > 0)
    if (!crc64_riscv_init_done) crc64_riscv_init_tables();
    if (len < 128)
        return crc64_slice8(crc, buf, len);
    return crc64_riscv_zbc_impl(crc, buf, len);
#else
