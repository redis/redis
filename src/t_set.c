...
void saddCommand(client *c) {
    ...
    if (added) {
        ...
        signalModifiedKey(c,c->db,c->argv[1],set);
...
void sremCommand(client *c) {
    ...
    if (deleted) {
        ...
        signalModifiedKey(c, c->db, c->argv[1], keyremoved ? NULL : set);
...
void smoveCommand(client *c) {
    ...
    signalModifiedKey(c, c->db, c->argv[1], (srcNewLen > 0) ? srcset : NULL);
    server.dirty++;
    ...
    if (setTypeAdd(dstset,ele->ptr)) {
        ...
        server.dirty++;
        signalModifiedKey(c,c->db,c->argv[2],dstset);
...
void spopCommand(client *c) {
    ...
    signalModifiedKey(c, c->db, c->argv[1], deleted ? NULL : kv);
    server.dirty++;
}
    ...
    if (empty > 0) {
        ...
        if (dstkey) {
            if (dbDelete(c->db,dstkey)) {
                signalModifiedKey(c,c->db,dstkey,NULL);
