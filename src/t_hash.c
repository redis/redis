...
/*-----------------------------------------------------------------------------
 * listpackTTL functions
 *----------------------------------------------------------------------------*/
/*
 * If 'hexpire' command is called on a listpack hash object for the first time,
 * we convert it to OBJ_ENCODING_LISTPACK_TTL encoding. Each field-value pair
 * will have an additional TTL entry. So, listpack will have triplets in it:
...
static void listpackTTLFree(listpackTTL *lpt) {
    lpFree(lpt->lp);
    zfree(lpt);
}
...
static uint64_t listpackTTLExpireDryRun(const robj *o) {
    ...
    while (fptr != NULL) {
        ...
        if (!hashTypeListpackIsExpired(val))
            break;
...
static uint64_t listpackTTLGetMinExpire(robj *o) {
    serverAssert(o->encoding == OBJ_ENCODING_LISTPACK_TTL);
...
static void listpackTTLExpire(robj *o, ExpireInfo *info) {
    serverAssert(o->encoding == OBJ_ENCODING_LISTPACK_TTL);
...
static void listpackTTLPersist(robj *o, sds field, unsigned char *fptr,
                              unsigned char *vptr)
{
    serverAssert(o->encoding == OBJ_ENCODING_LISTPACK_TTL);
 ...
 * by expiry time. */
static void listpackTTLUpdateExpiry(robj *o, sds field,
                                    unsigned char *fptr,
                                    unsigned char *vptr,
                                    uint64_t expireAt) {
    unsigned int slen;
...
static void listpackTTLAddNew(robj *o, sds field, sds value, uint64_t expireAt) {
                                   ...
                                   unsigned char *tptr, uint64_t expireAt)
{
    ...
    listpackTTLUpdateExpiry(ex->hashObj, field, fptr, vptr, expireAt);
    ex->fieldUpdated++;
    return HSETEX_OK;
}
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (fptr != NULL) {
            ...
            if (fptr != NULL) {
                ...
                if (hashTypeListpackIsExpired(expire))
                    return -1;
            }
        }
    }
                      ...
                      HashTypeSetEx *ex)
{
    ...
    if (ex->hashObj->encoding == OBJ_ENCODING_LISTPACK) {
        hashTypeConvert(c->db, ex->hashObj, OBJ_ENCODING_LISTPACK_TTL);

        listpackTTL *lpt = ex->hashObj->ptr;
...
unsigned long hashTypeLength(const robj *o, int subtractExpiredFields) {
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        if (subtractExpiredFields && lpt->meta.trash == 0)
            length -= listpackTTLExpireDryRun(o);
    } else if (o->encoding == OBJ_ENCODING_HT) {
...
hashTypeIterator *hashTypeInitIterator(robj *subject) {
    ...
    if (hi->encoding == OBJ_ENCODING_LISTPACK ||
        hi->encoding == OBJ_ENCODING_LISTPACK_TTL)
    {
        hi->fptr = NULL;
        hi->vptr = NULL;
        hi->tptr = NULL;
...
int hashTypeNext(hashTypeIterator *hi, int skipExpiredFields) {
    if (hi->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        while (fptr != NULL) {
            ...
            if (!skipExpiredFields || !hashTypeListpackIsExpired(expire_time))
                break;
...
void hashTypeConvertListpack(robj *o, int enc) {
    ...
    if (enc == OBJ_ENCODING_LISTPACK) {
        ...
        listpackTTL *lpt = listpackTTLCreate();
        lpt->lp = o->ptr;
        o->encoding = OBJ_ENCODING_LISTPACK_TTL;
        o->ptr = lpt;
...
void hashTypeConvertListpackTTL(robj *o, int enc, redisDb *db) {
    ...
    if (enc == OBJ_ENCODING_LISTPACK_TTL) {
        ...
        if (db && minExpire != EB_EXPIRE_TIME_INVALID)
...
robj *hashTypeDup(robj *o, sds newkey, uint64_t *minHashExpire) {
    ...
    if(o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        listpackTTL *dup = listpackTTLCreate();
        dup->lp = lpNew(sz);
        dup->key = newkey;
...
static ExpireAction hashTypeActiveExpire(eItem _hashObj, void *ctx) {
    ...
    if (hashObj->encoding == OBJ_ENCODING_LISTPACK_TTL) {
        ...
        listpackTTLExpire(hashObj, &info);
        keystr = ((listpackTTL*)hashObj->ptr)->key;
    } else {
...
uint64_t hashTypeGetNextTimeToExpire(robj *o) {
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        return listpackTTLGetMinExpire(o);
    } else {
        serverAssert(o->encoding == OBJ_ENCODING_HT);
...
void hpersistCommand(client *c) {
    ...
    if (hashObj->encoding == OBJ_ENCODING_LISTPACK) {
        ...
        for (int i = 0 ; i < numFields ; i++) {
            ...
            listpackTTLPersist(hashObj, field, fptr, vptr);
...
#define HSETF_FIELD_AND_TTL  2       /* Both field value and TTL is set */
                                       ...
                                       uint64_t expireAt, uint64_t *minPrevExp)
{
    ...
    if (o->encoding == OBJ_ENCODING_HT) {
        ...
        if (fptr == NULL || hashTypeListpackIsExpired(expire)) {
            addReplyNull(c);
            return 0;
        }
    ...
    /* Check if any of EX, EXAT, PX, PXAT, PERSIST flags is set */
    if (!(flag & (HFE_CMD_EXPIRY_MASK))) {
        return 0;
    }

    if (prevExpire == EB_EXPIRE_TIME_INVALID) {
        if (flag & (HFE_CMD_XX | HFE_CMD_LT | HFE_CMD_GT | HFE_CMD_PERSIST))
            return 0;
    } else {
        if (((flag & HFE_CMD_GT) && (prevExpire >= expireAt)) ||
            ((flag & HFE_CMD_LT) && (prevExpire <= expireAt)) ||
            (flag & HFE_CMD_NX))
            return 0;
...
/* Convert listpack to OBJ_ENCODING_LISTPACK_TTL or attach hfe meta to dict */
static void attachHfeMeta(redisDb *db, robj *o, robj *keyArg) {
    if (o->encoding == OBJ_ENCODING_LISTPACK) {
        hashTypeConvert(db, o, OBJ_ENCODING_LISTPACK_TTL);
...
/* Set fields and  expire time according to 'flag'.
                                 ...
                                 int flag, uint64_t expireAt, uint64_t *minPrevExp)
{
    ...
    if (o->encoding == OBJ_ENCODING_LISTPACK_TTL) {
        ...
        long long vll = LLONG_MAX;
        long long expire;
        unsigned char *fptr, *vptr = NULL, *tptr, *h, *vstr;
        listpackTTL *lpt = o->ptr;
        ...
        if (fptr == NULL) { /* Field does not exist */
            int ret = HSETF_FAIL;

            /* Check if "don't create field" is set */
            if (flag & HFE_CMD_DCF) {
            ...
            if (flag & (HFE_CMD_EX | HFE_CMD_EXAT | HFE_CMD_PX | HFE_CMD_PXAT)) {
                /* As the field does not exist, add expiry time if there is none
                 * of NX/XX/LT/GT flags exist or NX flag is set. */
                if (flag & HFE_CMD_NX || (flag & HFE_CMD_COND_MASK) == 0)
                    ret = HSETF_FIELD_AND_TTL; /* Both value and ttl will be set */
            }
            ...
            if (ret != HSETF_FIELD_AND_TTL) {
                listpackTTLAddNew(o, field, value, HASH_LP_NO_TTL);
            } else {
                /* If expiration time is in the past, no need to create the field */
                if (checkAlreadyExpired(expireAt))
                    return 1;

                listpackTTLAddNew(o, field, value, expireAt);
            }
            ...
            /* Check if "don't override field" flag is set. */
            if (flag & HFE_CMD_DOF) {
            ...
            /* If requested, reply with old value before overriding it. */
            if (flag & HFE_CMD_GETOLD) {
                vstr = lpGetValue(vptr, &vlen, &vll);
                if (vstr)
                    addReplyBulkCBuffer(c, vstr, vlen);
            ...
            if (prevExpire == EB_EXPIRE_TIME_INVALID) {
                if (flag & (HFE_CMD_EX | HFE_CMD_EXAT | HFE_CMD_PX | HFE_CMD_PXAT))
                    /* Update expiry time if none of NX/XX/LT/GT flags exist or
                     * NX flag is set. TODO: LT should be here. */
                    if (flag & (HFE_CMD_NX) || (flag & HFE_CMD_COND_MASK) == 0)
                        ret = HSETF_FIELD_AND_TTL;
            } else {
                /* If none of EX,EXAT,PX,PXAT,KEEPTTL is specified, TTL is
                 * discarded. In that case, return value should be
                 * HSETF_FIELD_AND_TTL as well*/
                if ((!(flag & HFE_CMD_EXPIRY_MASK)) ||
            ...
            if (flag & HFE_CMD_GETOLD)
                ; /* If GETOLD flag is used, replied above */
            else if (flag & HFE_CMD_GETNEW)
                addReplyBulkCBuffer(c, value, sdslen(value));
            else
                addReplyLongLong(c, ret);
            ...
            if (ret == HSETF_FIELD_AND_TTL) {
                *minPrevExp = min(*minPrevExp, prevExpire);
                ...
                if (!(flag & HFE_CMD_EXPIRY_MASK)) {
                    ...
                    *minPrevExp = min(*minPrevExp, expireAt);
                    listpackTTLUpdateExpiry(o, field, fptr, vptr, expireAt);
                }
            }
        ...
        if (de == NULL) {
            ...
            /* Check if "don't create field" flag is set. */
            if (flag & HFE_CMD_DCF) {
                if (flag & (HFE_CMD_GETNEW | HFE_CMD_GETOLD))
                    addReplyNull(c);
                else
            ...
            ret = HSETF_FIELD;
            /* Decide if we are going to update TTL */
            if (flag & (HFE_CMD_EX | HFE_CMD_EXAT | HFE_CMD_PX | HFE_CMD_PXAT)) {
                /* As the field does not exist, add expiry time if there is none
                 * of NX/XX/LT/GT flags exist or NX flag is set.
                 * TODO: LT should be here. */
                if (flag & HFE_CMD_NX || (flag & HFE_CMD_COND_MASK) == 0) {
                    ret = HSETF_FIELD_AND_TTL;
                }
            }
            ...
            /* If expiration time is in the past, no need to create field */
            if (ret == HSETF_FIELD_AND_TTL && checkAlreadyExpired(expireAt))
                return 1;

            int withExpireMeta = (ret == HSETF_FIELD_AND_TTL) ? 1 : 0;
            hfield hfNew = hfieldNew(field, sdslen(field), withExpireMeta);

            dictUseStoredKeyApi(d, 1);
            de = dictAddRaw(d, hfNew, NULL);
            dictUseStoredKeyApi(d, 0);
            ...
            if (ret == HSETF_FIELD_AND_TTL) {
                *minPrevExp = min(*minPrevExp, expireAt);
                ebAdd(&meta->hfe, &hashFieldExpireBucketsType, hfNew, expireAt);
            }
            ...
            hf = dictGetKey(de);
            prevExpire = hfieldGetExpireTime(hf);
            sds prevVal = dictGetVal(de);

            /* Check if "don't overwrite field" flag is set. */
            if (flag & HFE_CMD_DOF) {
                if (flag & (HFE_CMD_GETNEW | HFE_CMD_GETOLD))
            ...
            if (prevExpire == EB_EXPIRE_TIME_INVALID) {
                if (flag & (HFE_CMD_EX | HFE_CMD_EXAT | HFE_CMD_PX | HFE_CMD_PXAT)) {
                    /* Update expiry time if there is none of NX/XX/LT/GT flags
                     * exist or NX flag is set. */
                    if (flag & (HFE_CMD_NX) || (flag & HFE_CMD_COND_MASK) == 0)
                        ret = HSETF_FIELD_AND_TTL;
                }
            } else {
            ...
            if (ret == HSETF_FIELD_AND_TTL) {
                *minPrevExp = min(*minPrevExp, prevExpire);
                ...
                if (!(flag & HFE_CMD_EXPIRY_MASK)) {
                    ...
                    if (!hfieldIsExpireAttached(hf)) {
                        /* allocate new field with expire metadata */
                        hfield hfNew = hfieldNew(hf, hfieldlen(hf), 1);
                        /* Replace the old field with the new one with metadata */
                        dictSetKey(d, de, hfNew);
                        hfieldFree(hf);
                        hf = hfNew;
