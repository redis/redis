...
static void populateDeltaHistograms(kvstore *kvs, asmTrimCtx *ctx) {
    ...
    while ((de = kvstoreIteratorNext(&kvs_it)) != NULL) {
        ...
        int sizeBin = keysizesHistBin(len); /* Strings, streams and bitmaps can be empty */
        keysizesRow[sizeBin]++;
        ...
        if (server.memory_tracking_enabled) {
            ...
            int allocBin = keysizesHistBin(alloc_size);
...
size_t lazyfreeGetFreeEffort(robj *key, robj *obj, int dbid) {
    if (obj->type == OBJ_LIST && obj->encoding == OBJ_ENCODING_QUICKLIST) {
        ...
        size_t containers = bitroarContainerCount(obj);
