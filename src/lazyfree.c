...
/* Release the lua script dicts. */
void lazyFreeLuaScripts(void *args[]) {
    dict *lua_eval_scripts = args[0];
    dict *lua_load_scripts = args[1];
    lua_State *lua = args[2];
    long long len = dictSize(lua_eval_scripts) + dictSize(lua_load_scripts);
    freeLuaScriptsSync(lua_eval_scripts, lua_load_scripts, lua);
    atomicDecr(lazyfree_objects,len);
    atomicIncr(lazyfreed_objects,len);
}
...
void freeLuaScriptsAsync(dict *lua_eval_scripts, dict *lua_load_scripts, lua_State *lua) {
    ...
    if (len > LAZYFREE_THRESHOLD) {
        atomicIncr(lazyfree_objects,len);
        bioCreateLazyFreeJob(lazyFreeLuaScripts,3,lua_eval_scripts,lua_load_scripts,lua);
    } else {
        freeLuaScriptsSync(lua_eval_scripts, lua_load_scripts, lua);
    }
}
