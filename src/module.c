 ...
 *                                            migration.
 *  * REDISMODULE_CTX_FLAGS_SLOT_MIGRATION_IMPORT: The command is being executed as part of
 *                                                 importing slots during a slot migration.
 */
    ...
    if (ctx) {
        if (ctx->client) {
            ...
            if (asmIsExecutingTask(ctx->client))
                flags |= REDISMODULE_CTX_FLAGS_SLOT_MIGRATION_IMPORT;
        }
