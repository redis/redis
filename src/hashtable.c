...
static size_t test_mem_usage;

/* Init hash function salt and seed random generators. */
...
static void freekeyval(hashtable *ht, void *keyval) {
    UNUSED(ht);
    free(keyval);
}
...
static void emptyCallback(hashtable *ht) {
    UNUSED(ht);
    empty_callback_call_counter++;
}

static void add_find_delete_test_helper(int accurate) {
    int count = accurate ? 1000000 : 200;
    assert(test_mem_usage == 0);
    ...
    for (j = 0; j < count; j++) {
        ...
        assert(hashtableAdd(ht, e));
    }
    assert(hashtableMemUsage(ht) == test_mem_usage);
    ...
    for (j = 0; j < count; j++) {
        ...
        snprintf(val, sizeof(val), "%d", count - j + 42);
        void *found;
        assert(hashtableFind(ht, key, &found));
        keyval *e = found;
        assert(strcmp(getval(e), val) == 0);
    }
    ...
    for (j = 0; j < count / 2; j++) {
        ...
        if (j % 3 == 0) {
            ...
            snprintf(val, sizeof(val), "%d", count - j + 42);
            void *popped;
            assert(hashtablePop(ht, key, &popped));
            keyval *e = popped;
            assert(strcmp(getval(e), val) == 0);
            free(e);
        } else {
            assert(hashtableDelete(ht, key));
        }
    }
    assert(hashtableMemUsage(ht) == test_mem_usage);
...
static uint64_t mock_hash_entry_get_hash(const void *entry) {
    ...
    const mock_hash_entry *mock = entry;
    return (mock->hash != 0) ? mock->hash : mock->value;
}

/* Scan contract helpers: detect duplicates using a per-value seen counter.
 * Values scanned are longs in the range [1, capacity). */
typedef struct {
    uint8_t *seen; /* one counter per possible value */
    long capacity;
    long duplicates;
    long unique;
} scan_contract_data;
...
static void scanContractFn(void *privdata, void *entry) {
    scan_contract_data *data = privdata;
    unsigned long val = (unsigned long)entry;
    assert(val < (unsigned long)data->capacity);
    if (data->seen[val]) {
        data->duplicates++;
    } else {
        data->unique++;
...
static scan_contract_data *create_scan_contract_data(long capacity) {
    scan_contract_data *data = calloc(1, sizeof(scan_contract_data));
    data->seen = calloc(capacity, 1);
    data->capacity = capacity;
    return data;
}

static void free_scan_contract_data(scan_contract_data *data) {
    free(data->seen);
    free(data);
}
...
static void populateSequential(hashtable *ht, long count) {
    for (long j = 1; j <= count; j++) {
        assert(hashtableAdd(ht, (void *)j));
    }
}

/* Finish any in-progress rehashing. */
...
/* ./redis-server test hashtable [--accurate] [--large-memory] */
int hashtableTest(int argc, char **argv, int flags) {
    UNUSED(argc);
    UNUSED(argv);
    int accurate = (flags & REDIS_TEST_ACCURATE);
    int large_memory = (flags & REDIS_TEST_LARGE_MEMORY);

    TEST("cursor") {
        assert(nextCursor(0x0000, 0xffff) == 0x8000);
        assert(nextCursor(0x8000, 0xffff) == 0x4000);
        assert(nextCursor(0x4001, 0xffff) == 0xc001);
        assert(nextCursor(0xffff, 0xffff) == 0x0000);
    }

    TEST("set hash function seed") {
        randomSeed();
    }

    TEST("add, find, delete") {
        size_t used_memory_before = zmalloc_used_memory();
        add_find_delete_test_helper(accurate);
        assert(zmalloc_used_memory() == used_memory_before);
    }

    TEST("add, find, delete avoid resize") {
        size_t used_memory_before = zmalloc_used_memory();
    ...
    TEST("instant rehashing") {
        ...
        for (j = 0; j < count; j++) {
            assert(hashtableDelete(ht, (void *)j));
            assert(!hashtableIsRehashing(ht));
        }
    ...
    TEST("rehashing with many empty buckets") {
        ...
        while (hashtableIsRehashing(ht)) {
            assert(!hashtableAdd(ht, (void *)0));
        }

        /* Keep the entry from the highest bucket index so shrink rehashing
         * doesn't complete too early with randomized hash seeds and start a
         * second resize. */
        ...
        /* Delete all entries except one so there are a lot of empty buckets. */
        hashtablePauseAutoShrink(ht);
        for (j = 0; j < 1000; j++) {
            if (j == keep) continue;
            assert(hashtableDelete(ht, (void *)j));
        ...
        hashtableResumeAutoShrink(ht);
        assert(hashtableSize(ht) == 1);
        assert(hashtableGetRehashingIndex(ht) == 0);

        /* Add entries to drive rehashing forward. A rehash step visits a
         * maximum of 10 empty buckets. Too few entries are added to overload
         * the new table, so the shrink is not aborted. */
        for (j = 0; j < 10; j++) {
            assert(hashtableAdd(ht, (void *)(1000 + j)));
        }
        assert(hashtableSize(ht) == 11);
        /* Check that at least 90 buckets are rehashed or that rehashing is
         * completed. */
        assert(hashtableGetRehashingIndex(ht) >= 90 || hashtableGetRehashingIndex(ht) == -1);

        hashtableRelease(ht);
    }

    TEST("shrink rehashing abort") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        long j;
        long keep = 0;
        ...
        /* Add entries to reach MAX_FILL_PERCENT_HARD in the new table, which
         * triggers the shrink rehashing to abort. */
        long add = hashtableEntriesPerBucket() * 5;
        for (j = 0; j < add; j++) {
            assert(hashtableAdd(ht, (void *)(20000 + j)));
        }

        /* Check that we restart the rehashing. */
        assert(hashtableSize(ht) == (size_t)(add + 1));
        assert(hashtableGetRehashingIndex(ht) == 0);
    ...
    TEST("bucket chain length") {
        ...
        while (hashtableIsRehashing(ht)) {
            assert(!hashtableExpand(ht, count * 2));
            j++;
            assert(hashtableAdd(ht, (void *)j));
        }
        assert(j < count * 2);
        int max_chainlen_not_rehashing = hashtableLongestBucketChain(ht);
        assert(max_chainlen_not_rehashing < 10);
        ...
        assert(max_chainlen_rehashing < 10);

        hashtableRelease(ht);
    }

    TEST("two-phase insert and pop") {
        int count = accurate ? 1000000 : 200;
        hashtable *ht = hashtableCreate(&keyval_type);
        int j;
        ...
        for (j = 0; j < count; j++) {
            ...
            keyval *e = *ref;
            assert(strcmp(getval(e), val) == 0);
            assert(hashtableSize(ht) == size_before_find);
            hashtableTwoPhasePopDelete(ht, &position);
            assert(hashtableSize(ht) == size_before_find - 1);
            free(e);
        }
    ...
    TEST("replace reallocated entry") {
        size_t used_memory_before = zmalloc_used_memory();
        ...
        for (j = 0; j < count; j++) {
            ...
            assert(hashtableFind(ht, key, &found));
            keyval *old = found;
            assert(strcmp(getkey(old), key) == 0);
            assert(strcmp(getval(old), val) == 0);
            snprintf(val, sizeof(val), "%d", j + 1234);
            keyval *new_entry = create_keyval(key, val);
            /* If we free 'old' before the call to
             * hashtableReplaceReallocatedEntry, we get a use-after-free
             * warning, so instead we just overwrite it with junk. The purpose
             * is to verify that the function doesn't use the memory it points
             * to. */
            memset(old->data, 'x', old->keysize + old->valsize);
            assert(hashtableReplaceReallocatedEntry(ht, old, new_entry));
            free(old);
        }
    ...
    TEST("incremental find") {
        size_t count = 2000000;
        uint8_t *element_array = malloc(count);
        ...
        for (size_t j = 0; j < count; j++) {
            assert(hashtableAdd(ht, element_array + j));
        }
        ...
        for (size_t batch_size = 1; batch_size <= 64; batch_size *= 2) {
            ...
            for (size_t batch = 0; batch < count / batch_size; batch++) {
                /* Init batches. */
                hashtableIncrementalFindState *states =
                ...
                for (size_t i = 0; i < batch_size; i++) {
                    void *found;
                    assert(hashtableIncrementalFindGetResult(&states[i], &found));
                    assert(found == &element_array[batch * batch_size + i]);
                }
                free(states);
            }
    ...
    TEST("scan") {
        ...
        for (int round = 0; round < num_rounds; round++) {
            ...
            assert(scanned_count == count);
            for (j = 0; j < count; j++) {
                assert(data->entry_seen[j] >= 1);
                assert(data->entry_seen[j] <= 2);
            }
    ...
    TEST("iterator") {
        size_t count = 2000000;
        uint8_t *entry_array = malloc(count);
        ...
        /* Populate */
        for (size_t j = 0; j < count; j++) {
            assert(hashtableAdd(ht, entry_array + j));
        }
        ...
        while (hashtableNext(&iter, &next)) {
            uint8_t *entry = next;
            num_returned++;
            assert(entry >= entry_array);
            assert(entry < entry_array + count);
        ...
        assert(num_returned == count);
        for (size_t j = 0; j < count; j++) {
            assert(entry_array[j] == 1);
        }
    ...
    TEST("safe iterator") {
        size_t count = 1000;
        uint8_t *entry_counts = malloc(count);
        ...
        /* Populate */
        for (size_t j = 0; j < count; j++) {
            assert(hashtableAdd(ht, entry_counts + j));
        }
        ...
        while (hashtableNext(&iter, &next)) {
            ...
            num_returned++;
            assert(entry >= entry_counts);
            assert(entry < entry_counts + count);
            /* increment entry at this position as a counter */
            (*entry)++;
        ...
        assert(num_returned == count);
        for (size_t j = 0; j < count; j++) {
            assert(entry_counts[j] == 1);
        }

        /* Verify correct number of entries remain */
        assert(hashtableSize(ht) == count - count / 4);

        hashtableRelease(ht);
        free(entry_counts);
    }

    TEST("compact bucket chain") {
        size_t used_memory_before = zmalloc_used_memory();
        ...
        while (hashtableNext(&iter, &entry)) {
             ...
             * bucket chain, the bucket chain is not compacted, so it still has
             * the same number of buckets. */
            assert(hashtableChainedBuckets(ht, 0) == num_chained_buckets);
            num_returned++;
            if (num_returned % 2 == 0) {
                assert(hashtableDelete(ht, entry));
            }
        ...
        /* Verify that the bucket chain has been compacted by filling the holes
         * and freeing empty child buckets. */
    ...
    TEST("random entry fairness") {
        ...
        for (size_t j = 0; j < count; j++) {
            assert(hashtableAdd(ht, times_picked + j));
        }

        /* Pick entries, and count how many times each entry is picked. */
        for (long i = 0; i < num_rounds; i++) {
            /* Using void* variable to avoid a cast that violates strict
             * aliasing */
            void *entry;
            assert(hashtableFairRandomEntry(ht, &entry));
            unsigned *picked = entry;
            assert(picked >= times_picked);
            assert(picked < times_picked + count);
            /* increment entry at this position as a counter */
            (*picked)++;
        }
        hashtableRelease(ht);
         ...
         * --------------------
         *
         * Selecting a single random entry: For any entry in the hash table,
         * let X=1 if the we selected the entry (success) and X=0 otherwise.
         * With m entries, our entry is sepected with probability p = 1/m, the
         * expected value is E(X) = 1/m, E(X^2) = 1/m and the variance:
         *
         *     Var(X) = E(X^2) - (E(X))^2 = 1/m - 1/(m^2) = (1/m) * (1 - 1/m).
         *
         * Repeating the selection of a random entry: Let's repeat the
         * experiment n times and let Y be the number of times our entry was
         * selected. This is a binomial distribution.
         *
         *     Y = X_1 + X_2 + ... + X_n
         *     E(Y) = n/m
         *
         * The variance of a sum of independent random variables is the sum of
         * the variances, so Y has variance np(1−p).
         *
        ...
        /* With large n, the distribution approaches a normal distribution and
         * we can use p68 = within 1 std dev, p95 = within 2 std dev, p99.7 =
         * within 3 std dev. */
        ...
        /* Conclusion? The number of trials (n) relative to the probabilities
         * (p and 1 − p) must be sufficiently large (n * p ≥ 5 and
         * n * (1 − p) ≥ 5) to approximate a binomial distribution with a
         * normal distribution. */
        if (n / m >= 5 && n * (1 - 1 / m) >= 5) {
            /* Check that 80% of the elements are picked within 3 std
             * deviations of the expected number. This is a low bar, since
             * typically the 99% of the elements are within this range.
             *
             * There is an edge case. When n is very large and m is very small,
             * the std dev of a binomial distribution is very small, which
             * becomes too strict for our bucket layout and makes the test
             * flaky. For example with m = 400 and n = 1M, we get an expected
             * value of 2500 and a std dev of 50, which is just 2% of the
             * expected value. We lower the bar for this case and accept that
             * 80% of elements are just within 10% of the expected value. */
            assert(100 * p99 / m >= 80.0 || 100 * p10percent / m >= 80.0);
        } else {
            printf("To uncertain numbers to draw any conclusions about fairness.\n");
        }
    }

    TEST("random entry with long chain") {
        ...
        /* This is confidence level for our measurement as the Z value of a
         * normal distribution. 5 sigma corresponds to 0.00002% probability
         * that our measurement is farther than 'precision' from the truth.
         * This value is used in particle physics. */
        const double z = 5;
    ...
    TEST("safe iterator invalidation on release") {
        ...
        /* Add some entries */
        for (int i = 0; i < 10; i++) {
            assert(hashtableAdd(ht, (void *)(long)i));
        }
        ...
        void *entry;
        assert(hashtableNext(&safe_iter1, &entry));
        assert(hashtableNext(&safe_iter2, &entry));
    ...
    TEST("hashtableEmpty does not invalidate safe iterators") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        /* Iterator should still be valid but return false since table is
         * empty */
        void *entry;
        assert(!hashtableNext(&safe_iter, &entry));

        hashtableCleanupIterator(&safe_iter);
        hashtableRelease(ht);
    }

    TEST("safe iterator cleanup and invalidation") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
    ...
    TEST("safe iterator cleanup untracking") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        /* Release hashtable - iterator should not be invalidated since it was
         * reset */
        hashtableRelease(ht);
    ...
    TEST("safe iterator pause/resume rehashing tracking") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        /* Start iterating with first iterator - should pause rehashing and
         * track iterator */
        void *entry;
        assert(hashtableNext(&iter1, &entry));

        /* Start iterating with second iterator - should also be tracked */
        assert(hashtableNext(&iter2, &entry));

        /* Verify rehashing is paused (safe iterators should pause it) */
        assert(hashtableIsRehashingPaused(ht));

        /* Reset first iterator - should untrack it but rehashing still paused
         * due to iter2 */
        hashtableCleanupIterator(&iter1);

        /* Start third iterator */
        assert(hashtableNext(&iter3, &entry));

        /* Reset second iterator - rehashing should still be paused due to
         * iter3 */
        ...
        /* Verify all iterators are properly untracked by releasing
         * hashtable */
        hashtableRelease(ht);
    }

    TEST("NULL hashtable iterator") {
        ...
        /* hashtableNext should return false for NULL hashtable */
        assert(!hashtableNext(&unsafe_iter, &entry));
    ...
    TEST("retarget iterator") {
        ...
        for (int i = 0; i < 5; i++) {
            assert(hashtableAdd(ht1, (void *)(long)(i + 10)));
            assert(hashtableAdd(ht2, (void *)(long)(i + 20)));
        }
    ...
    TEST("iterator next after exhaustion") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
    ...
    TEST("safe iterator cleanup resumes rehashing after exhaustion") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);

        /* Add entries until rehashing starts so we can observe
         * pause/resume. */
        long j = 0;
        while (!hashtableIsRehashing(ht)) {
            j++;
        ...
        /* Exhaust the iterator. Rehashing stays paused until the iterator is
         * cleaned up. */
        while (hashtableNext(&iter, &entry)) {
        }
        assert(hashtableIsRehashingPaused(ht));
    ...
    TEST("safe iterator release before cleanup") {
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        /* Release the hashtable before calling cleanup. The iterator is
         * invalidated by the release, so this should not access freed
         * memory. */
        hashtableRelease(ht);
    ...
    TEST("scan no duplicates in a static table") {
        ...
        hashtablePauseAutoShrink(ht);
        scan_contract_data *data = create_scan_contract_data(count + 1);
        size_t cursor = 0;
        do {
        ...
        assert(data->unique == count);

        free_scan_contract_data(data);
        hashtableRelease(ht);
    }

    TEST("scan no duplicates with deletions") {
        /* Scan with deletions mid-scan: no duplicates, some entries may be
         * missed. */
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
    ...
    TEST("scan no duplicates with insertions") {
        ...
        hashtablePauseAutoShrink(ht);
        scan_contract_data *data = create_scan_contract_data(count * 8);
        size_t cursor = 0;
    ...
    TEST("scan no duplicates fuzz") {
        /* Fuzz test: random seed, random ops during scan, assert no
         * duplicates. */
        unsigned int fuzz_seed = (unsigned int)time(NULL);
        printf("scan no duplicates fuzz seed: %u\n", fuzz_seed);
        srand(fuzz_seed);
        int iterations = 50;
    ...
    TEST("scan no shrink during scan callback") {
        /* Verify that shrink doesn't start during a scan callback even when
         * the fill factor drops below the threshold. */
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        hashtableScan(ht, 0, scanDeleteManyCb, ht);

        /* Shrink is still blocked by PauseAutoShrink. */
        assert(hashtableSize(ht) < size_before / 2);
        assert(!hashtableIsRehashing(ht));
    ...
    TEST("scan no duplicates during expand rehash") {
        /* Verify no duplicates when expansion starts and rehashing completes
         * during an ongoing scan. Lookups between scan calls drive rehash
         * steps. */
        hashtableType type = {0};
        hashtable *ht = hashtableCreate(&type);
        ...
        /* Continue scanning while driving rehash to completion with
         * lookups. */
        ...
        assert(rehash_completed_during_scan);
        assert(data->duplicates == 0);
