...
static int zslParseRange(robj *min, robj *max, zrangespec *spec) {
    ...
    if (min->encoding == OBJ_ENCODING_INT) {
        ...
        if (((char*)min->ptr)[0] == '(') {
            spec->min = fast_float_strtod((char*)min->ptr+1,sdslen(min->ptr)-1,&eptr);
            if (eptr[0] != '\0' || isnan(spec->min)) return C_ERR;
            spec->minex = 1;
        } else {
            spec->min = fast_float_strtod((char*)min->ptr,sdslen(min->ptr),&eptr);
            if (eptr[0] != '\0' || isnan(spec->min)) return C_ERR;
        }
    }
    if (max->encoding == OBJ_ENCODING_INT) {
        ...
        if (((char*)max->ptr)[0] == '(') {
            spec->max = fast_float_strtod((char*)max->ptr+1,sdslen(max->ptr)-1,&eptr);
            if (eptr[0] != '\0' || isnan(spec->max)) return C_ERR;
            spec->maxex = 1;
        } else {
            spec->max = fast_float_strtod((char*)max->ptr,sdslen(max->ptr),&eptr);
            if (eptr[0] != '\0' || isnan(spec->max)) return C_ERR;
        }
    }
