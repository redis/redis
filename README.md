...
- [Build Redis from source](#build-redis-from-source)
  ...
  - [Build](#build)
    ...
    - [Build flags](#build-flags)
      ...
      - [Query Engine with Intel SVS-VAMANA optimisations](#query-engine-with-intel-svs-vamana-optimisations)
  - [Troubleshooting](#troubleshooting)
    - [CMake version](#cmake-version)
    - [Rust toolchain](#rust-toolchain)
    - [Dependencies or cached build options](#dependencies-or-cached-build-options)
...
Redis is developed on GitHub at <https://github.com/redis/redis>. Either source below works — the release tarball already bundles the module sources, while a git checkout needs one extra step to fetch them:
...
`make modules-update` clones the bundled modules — JSON, time series, Bloom / cuckoo / count-min / top-k, t-digest, and the Query Engine — under `modules/*/src`, and generates the `redis-full.conf` you need to [run](#run) a git build. Skip it if you only want a core-only Redis.

### Prerequisites

Building Redis with all data structures (JSON, time series, Bloom / cuckoo / count-min / top-k, t-digest, and the Query Engine) needs a build toolchain plus a few version-sensitive dependencies — GCC/Clang, **LLVM 21**, **CMake 3.25–3.31.6**, **Rust 1.94**, OpenSSL, Python 3, and assorted `-dev` libraries. Instead of a per-OS package list, the repo installs them for you with **`make bootstrap`**, which detects your OS and installs each bundled module's prerequisites. If a version-sensitive dependency trips the build, see [Troubleshooting](#troubleshooting).
...
`make` (same as `make build` / `make all`) always builds whatever is cloned under `modules/*/src` alongside Redis core — there's no separate flag to opt in. If nothing is cloned yet (see [Sources](#sources)), you get a core-only build.

To build Redis with just the core data structures — even if the modules are already cloned — use:
...
Pass flags on the `make` command line (`make BUILD_TLS=yes`), or export them before building (`export BUILD_TLS=yes` then `make`).

| Flag | What it does |
| --- | --- |
| `BUILD_TLS=yes` | TLS support. Needs OpenSSL development libraries (e.g. `libssl-dev` on Debian/Ubuntu). See [Run](#run) for running and testing with TLS. |
| `USE_SYSTEMD=yes` | systemd support. Needs systemd development libraries (such as `libsystemd-dev` on Debian/Ubuntu or `systemd-devel` on CentOS). |
| `BUILD_COMPRESSION=yes` | Compression of the replication stream via zstd (supported as of 8.10). Needs zstd development libraries (e.g. `libzstd-dev` on Debian/Ubuntu). |
| `PROG_SUFFIX="-alt"` | Appends a suffix to the Redis program names. |
| `MALLOC=libc` / `MALLOC=jemalloc` | Selects the memory allocator — see [Allocator](#allocator). |
| `CFLAGS="-DUSE_PROCESSOR_CLOCK"` | Uses the processor's internal instruction clock as the monotonic clock source — see [Monotonic clock](#monotonic-clock). |
| `BUILD_INTEL_SVS_OPT=yes` | Intel SVS-VAMANA optimisations for the Query Engine — see [Query Engine with Intel SVS-VAMANA optimisations](#query-engine-with-intel-svs-vamana-optimisations). |
| `V=1` | More verbose build output — see [Verbose build](#verbose-build). |

A 32 bit binary is a separate target rather than a flag:
...
##### Query Engine with Intel SVS-VAMANA optimisations
...
The bundled modules include Rust code (the Query Engine among them) and are built against **Rust 1.94**. If your host Rust is older and a module build fails on the required `rustc` version, let the build provision the pinned toolchain instead of using your distro's:

```sh
make build INSTALL_RUST_TOOLCHAIN=yes
```

This downloads the Rust 1.94.0 standalone installer matching your architecture and libc (Linux x86_64 / aarch64, glibc or musl), verifies its SHA256, and installs it under `/usr/local` — no `rustup` involved. To install it without building anything, use `make -C modules install-rust INSTALL_RUST_TOOLCHAIN=yes`; to remove it afterwards:

```sh
make -C modules uninstall-rust INSTALL_RUST_TOOLCHAIN=yes
```

Rust and LLVM versions are coupled: link-time optimization in the Query Engine needs clang and lld from an LLVM version matching `rustc`, which is why **LLVM 21** goes with Rust 1.94, and why LTO is skipped on non-Linux hosts.

#### Dependencies or cached build options
...
Start the server with the configuration file that matches how you got the source:
...
> **Configuration files**: release tarballs bake the bundled modules'
...
> regenerated by `make sync-redis-conf`). Edit Redis-core settings in
> [`redis.conf`](redis.conf). See [modules/MODULES.md](modules/MODULES.md)
> for the full config flow.

While developing, `make run` is often quicker: it starts the server you just built with every cloned module auto-loaded via `--loadmodule` and no config file — pass extra options with `ARGS`, e.g. `make run ARGS="--port 6400"`, or name specific modules with `make run redisjson`.

To connect to it, see [Using Redis with redis-cli](#using-redis-with-redis-cli).

#### Running Redis with TLS

Build with [`BUILD_TLS=yes`](#build-flags) first. To run the test suite with TLS enabled (you will need `tcl-tls` installed):
