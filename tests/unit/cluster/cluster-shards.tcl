...
proc cluster_shards_hostnames_propagated {} {
    set node_count [expr {$::cluster_master_nodes + $::cluster_replica_nodes}]
    set ids {}
    for {set id 0} {$id < $node_count} {incr id} {
        lappend ids [R $id CLUSTER MYID]
    }

    for {set reference 0} {$reference < $node_count} {incr reference} {
        for {set id 0} {$id < $node_count} {incr id} {
            set node [cluster_shards_get_node_info [lindex $ids $id] $reference]
            if {![dict exists $node hostname] || [dict get $node hostname] ne "host-$id.com"} {
                return 0
            }
        }
    }
...
test "Set cluster hostnames and verify they are propagated" {
    ...
    wait_for_condition 50 100 {
        [cluster_shards_hostnames_propagated] eq 1
    } else {
        fail "cluster hostnames were not propagated to CLUSTER SHARDS"
    }
