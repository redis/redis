start_server {
    ...
    test "XNACK SILENT mode decrements delivery_count" {
        ...
        # delivery_count should be 1 after XREADGROUP
        set pending [r XPENDING mystream grp - + 10]
        assert_equal [lindex $pending 0 3] 1

        # XNACK with SILENT should decrement to 0
        assert_equal 1 [r XNACK mystream grp SILENT IDS 1 1-0]

        # Entry should still be in group PEL but with empty consumer and count 0
        set pending [r XPENDING mystream grp - + 10]
        assert_equal [llength $pending] 1
        assert_equal [lindex $pending 0 1] {}
    ...
    test "XNACK releases entries regardless of owning consumer" {
        ...
        r XADD mystream 2-0 f v2
        r XGROUP CREATE mystream grp 0
        # c1 gets 1-0, c2 gets 2-0
        r XREADGROUP GROUP grp c1 COUNT 1 STREAMS mystream >
        r XREADGROUP GROUP grp c2 COUNT 1 STREAMS mystream >

        # XNACK 2-0 (owned by c2) should succeed
        assert_equal 1 [r XNACK mystream grp FAIL IDS 1 2-0]

        # XNACK 1-0 (owned by c1) should succeed
        assert_equal 1 [r XNACK mystream grp FAIL IDS 1 1-0]

        # Both entries should now be unowned
        set pending [r XPENDING mystream grp - + 10]
    ...
    test "XNACK skips IDs not in group PEL" {
        ...
        # 9-9 is not in the PEL (and no FORCE, so skip)
    ...
    test "XNACK multiple IDs at once" {
        ...
        # All three should be in group PEL with empty consumer
        set pending [r XPENDING mystream grp - + 10]
    ...
    test "XNACK removes entry from consumer PEL" {
        ...
        # c1 should have 2 pending entries
        set c1_pending [r XPENDING mystream grp - + 10 c1]
        assert_equal [llength $c1_pending] 2
    ...
    test "XNACK NACKed entry can be reclaimed via XCLAIM" {
        ...
        # XCLAIM it as c2 with min-idle-time 0 (NACKed entries have delivery_time=0)
        set claimed [r XCLAIM mystream grp c2 0 1-0]
        assert_equal [llength $claimed] 1
        assert_equal [lindex $claimed 0 0] 1-0

        # c2 should now own the entry
    ...
    test "XNACK NACKed entry can be reclaimed via XAUTOCLAIM" {
        ...
        # XAUTOCLAIM with min-idle-time 0 should claim NACKed entries
    ...
    test "XNACK NACKed entry can be reclaimed via XREADGROUP CLAIM" {
        ...
        # XREADGROUP with CLAIM should pick up NACKed entry
    ...
    test "XNACK XACK works on NACKed (unowned) entry" {
        ...
        # XACK should still remove it from the group PEL
        assert_equal 1 [r XACK mystream grp 1-0]

        # Group PEL should now be empty
        set info [r XPENDING mystream grp]
        assert_equal [lindex $info 0] 0
    }
    ...
    test "XNACK RDB save and load preserves NACKed entries" {
        ...
        # Verify NACKed entries preserved
        set pending [r XPENDING mystream grp - + 10]
        assert_equal [llength $pending] 3

        # 1-0 and 2-0 should have empty consumer
        assert_equal [lindex $pending 0 0] 1-0
        assert_equal [lindex $pending 0 1] {}
        assert_equal [lindex $pending 1 0] 2-0
        assert_equal [lindex $pending 1 1] {}

        # 3-0 should still be owned by c1
        assert_equal [lindex $pending 2 0] 3-0
        assert_equal [lindex $pending 2 1] c1

        # Verify NACKed entries can still be claimed after reload
        set claimed [r XCLAIM mystream grp c2 0 1-0]
        assert_equal [llength $claimed] 1
        assert_equal [lindex $claimed 0 0] 1-0
    } {} {external:skip needs:debug}
    ...
    test "XNACK FATAL delivery_count survives RDB reload" {
        ...
        r XNACK mystream grp FATAL IDS 1 1-0
    ...
    start_server {tags {"stream needs:debug"} overrides {appendonly yes aof-use-rdb-preamble no appendfsync always}} {
        test "XNACK entries survive AOF rewrite" {
            ...
            # XNACK 1-0 (SILENT) and 3-0 (FAIL), keep 2-0 owned by c1
            r XNACK mystream grp SILENT IDS 1 1-0
            r XNACK mystream grp FAIL IDS 1 3-0

            # Verify state before rewrite
            set pending_before [r XPENDING mystream grp - + 10]
            ...
            # Verify NACKed entries survived
            set pending_after [r XPENDING mystream grp - + 10]
            assert_equal [llength $pending_after] 3

            # 1-0: NACKed (SILENT, delivery_count=0)
            assert_equal [lindex $pending_after 0 0] 1-0
            assert_equal [lindex $pending_after 0 1] {}
            assert_equal [lindex $pending_after 0 3] 0
            ...
            # 3-0: NACKed (FAIL, delivery_count=1)
    ...
    test "XNACK NACKed entries survive COPY" {
        r DEL mystream{t} mystream_copy{t}
        r XADD mystream{t} 1-0 f v1
        r XADD mystream{t} 2-0 f v2
        r XADD mystream{t} 3-0 f v3
        r XADD mystream{t} 4-0 f v4
        ...
        # Verify source state before COPY
        set info [r XINFO STREAM mystream{t} FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 2
        assert_equal [dict get $group pel-count] 4

        # COPY the stream
        assert_equal 1 [r COPY mystream{t} mystream_copy{t}]

        # Verify NACK zone in the copy
        set info_copy [r XINFO STREAM mystream_copy{t} FULL]
        set group_copy [lindex [dict get $info_copy groups] 0]
        assert_equal [dict get $group_copy nacked-count] 2
        assert_equal [dict get $group_copy pel-count] 4

        set pending [r XPENDING mystream_copy{t} grp - + 10]
        assert_equal [llength $pending] 4
        ...
        # 4-0: still owned by c1
        assert_equal [lindex $pending 3 0] 4-0
        assert_equal [lindex $pending 3 1] c1

        # Verify NACKed entries in copy are claimable
        set claimed [r XCLAIM mystream_copy{t} grp c2 0 1-0 3-0]
        assert_equal [llength $claimed] 2
    }

    test "XNACK NACKed entries survive COPY REPLACE" {
        r DEL src{t} dst{t}
        r XADD src{t} 1-0 f v1
        r XADD src{t} 2-0 f v2
        r XGROUP CREATE src{t} grp 0
        r XREADGROUP GROUP grp c1 STREAMS src{t} >

        r XNACK src{t} grp FAIL IDS 1 1-0

        # Create a pre-existing destination key
        r SET dst{t} "old_value"

        r COPY src{t} dst{t} REPLACE

        set info [r XINFO STREAM dst{t} FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 1
        assert_equal [dict get $group pel-count] 2

        set pending [r XPENDING dst{t} grp - + 10]
        assert_equal [lindex $pending 0 0] 1-0
        assert_equal [lindex $pending 0 1] {}
        assert_equal [lindex $pending 1 0] 2-0
        assert_equal [lindex $pending 1 1] c1
    ...
    test "XNACK NACKed entries survive RENAME" {
        r DEL mystream{t} mystream_renamed{t}
        r XADD mystream{t} 1-0 f v1
        r XADD mystream{t} 2-0 f v2
        r XADD mystream{t} 3-0 f v3
        r XGROUP CREATE mystream{t} grp 0
        ...
        r XNACK mystream{t} grp SILENT IDS 1 1-0
        r XNACK mystream{t} grp FAIL IDS 1 3-0

        # Verify state before RENAME
        set info [r XINFO STREAM mystream{t} FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 2

        r RENAME mystream{t} mystream_renamed{t}

        # Old key should not exist
        assert_equal 0 [r EXISTS mystream{t}]

        # Verify NACK zone in renamed key
        set info [r XINFO STREAM mystream_renamed{t} FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 2
        assert_equal [dict get $group pel-count] 3

        set pending [r XPENDING mystream_renamed{t} grp - + 10]
        assert_equal [llength $pending] 3

        # 1-0: NACKed (SILENT), unowned, delivery_count=0
        assert_equal [lindex $pending 0 0] 1-0
        assert_equal [lindex $pending 0 1] {}
        assert_equal [lindex $pending 0 3] 0
        ...
        # 3-0: NACKed (FAIL), unowned, delivery_count=1
        assert_equal [lindex $pending 2 0] 3-0
        assert_equal [lindex $pending 2 1] {}
        assert_equal [lindex $pending 2 3] 1

        # Verify NACKed entries are still claimable after rename
        set claimed [r XCLAIM mystream_renamed{t} grp c2 0 1-0]
        assert_equal [llength $claimed] 1
    }

    test "XNACK NACKed entries survive MOVE" {
        r SELECT 0
        r DEL mystream
        r XADD mystream 1-0 f v1
        r XADD mystream 2-0 f v2
        ...
        # Verify state before MOVE
        set info [r XINFO STREAM mystream FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 2
        assert_equal [dict get $group pel-count] 3

        # Clean destination DB and move
        r SELECT 9
        r DEL mystream
        r SELECT 0
        r MOVE mystream 9

        # Old key should not exist in DB 0
        assert_equal 0 [r EXISTS mystream]

        # Verify NACK zone in destination DB
        r SELECT 9
        set info [r XINFO STREAM mystream FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 2
        ...
        # Verify NACKed entries are claimable in new DB
        set claimed [r XCLAIM mystream grp c2 0 1-0 3-0]
        assert_equal [llength $claimed] 2

        # Clean up: remove from DB 9 and switch back
        r DEL mystream
        r SELECT 0
    } {OK} {singledb:skip}

    test "XNACK FORCE-created entries survive COPY" {
        r DEL mystream{t} mystream_copy{t}
        r XADD mystream{t} 1-0 f v1
        r XADD mystream{t} 2-0 f v2
        r XADD mystream{t} 3-0 f v3
        ...
        # Create NACKed entries via FORCE (no prior XREADGROUP)
        r XNACK mystream{t} grp FAIL IDS 1 1-0 FORCE
        r XNACK mystream{t} grp FATAL IDS 1 2-0 RETRYCOUNT 77 FORCE
        r XNACK mystream{t} grp SILENT IDS 1 3-0 RETRYCOUNT 5 FORCE
        ...
        r COPY mystream{t} mystream_copy{t}

        set info_copy [r XINFO STREAM mystream_copy{t} FULL]
        set group_copy [lindex [dict get $info_copy groups] 0]
        assert_equal [dict get $group_copy nacked-count] 3
        assert_equal [dict get $group_copy pel-count] 3
        ...
        assert_equal [lindex $pending 1 0] 2-0
        assert_equal [lindex $pending 1 1] {}
        assert_equal [lindex $pending 1 3] 77

        assert_equal [lindex $pending 2 0] 3-0
        assert_equal [lindex $pending 2 1] {}
        assert_equal [lindex $pending 2 3] 5
    }

    test "XNACK NACKed entries with multiple groups survive COPY" {
        r DEL mystream{t} mystream_copy{t}
        r XADD mystream{t} 1-0 f v1
        r XADD mystream{t} 2-0 f v2
        r XGROUP CREATE mystream{t} grp1 0
        r XGROUP CREATE mystream{t} grp2 0
        r XREADGROUP GROUP grp1 c1 STREAMS mystream{t} >
        r XREADGROUP GROUP grp2 c2 STREAMS mystream{t} >

        # NACK different entries in different groups
        r XNACK mystream{t} grp1 FAIL IDS 1 1-0
        r XNACK mystream{t} grp2 FAIL IDS 1 2-0

        r COPY mystream{t} mystream_copy{t}

        set info_copy [r XINFO STREAM mystream_copy{t} FULL]
        set grp1 [lindex [dict get $info_copy groups] 0]
        set grp2 [lindex [dict get $info_copy groups] 1]
        assert_equal [dict get $grp1 nacked-count] 1
        assert_equal [dict get $grp2 nacked-count] 1

        # Verify grp1 in the copy
        set pending1 [r XPENDING mystream_copy{t} grp1 - + 10]
        assert_equal [lindex $pending1 0 0] 1-0
        assert_equal [lindex $pending1 0 1] {}
        assert_equal [lindex $pending1 1 0] 2-0
        assert_equal [lindex $pending1 1 1] c1

        # Verify grp2 in the copy
        set pending2 [r XPENDING mystream_copy{t} grp2 - + 10]
        assert_equal [lindex $pending2 0 0] 1-0
        assert_equal [lindex $pending2 0 1] c2
    ...
    test "XNACK NACKed entries survive DUMP and RESTORE" {
        ...
        r XNACK mystream grp FATAL IDS 1 4-0

        # Verify state before DUMP
        set info [r XINFO STREAM mystream FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 3
        assert_equal [dict get $group pel-count] 4
        ...
        # Verify NACK zone survived round-trip
        set info [r XINFO STREAM mystream FULL]
        set group [lindex [dict get $info groups] 0]
        assert_equal [dict get $group nacked-count] 3
        assert_equal [dict get $group pel-count] 4
        ...
        # 2-0: NACKed (FAIL), unowned, delivery_count=1
        assert_equal [lindex $pending 1 0] 2-0
        assert_equal [lindex $pending 1 1] {}
        assert_equal [lindex $pending 1 3] 1

        # 3-0: still owned by c1
        assert_equal [lindex $pending 2 0] 3-0
        assert_equal [lindex $pending 2 1] c1

        # 4-0: NACKed (FATAL), unowned, delivery_count=LLONG_MAX
        assert_equal [lindex $pending 3 0] 4-0
        assert_equal [lindex $pending 3 1] {}
        assert_equal [lindex $pending 3 3] 9223372036854775807

        # Verify NACKed entries are claimable after restore
        set claimed [r XCLAIM mystream grp c2 0 1-0 2-0 4-0]
        assert_equal [llength $claimed] 3
    }

    test "XNACK FORCE-created entries survive DUMP and RESTORE" {
        r DEL mystream
        r XADD mystream 1-0 f v1
        r XADD mystream 2-0 f v2
        r XADD mystream 3-0 f v3
        ...
        r XNACK mystream grp FAIL IDS 1 1-0 FORCE
        r XNACK mystream grp FATAL IDS 1 2-0 RETRYCOUNT 42 FORCE
        r XNACK mystream grp SILENT IDS 1 3-0 RETRYCOUNT 7 FORCE
        ...
        assert_equal [lindex $pending 1 0] 2-0
        assert_equal [lindex $pending 1 1] {}
        assert_equal [lindex $pending 1 3] 42

        assert_equal [lindex $pending 2 0] 3-0
        assert_equal [lindex $pending 2 1] {}
        assert_equal [lindex $pending 2 3] 7
    }
    ...
    foreach diskless_load {"disabled" "swapdb"} {
        start_server {tags {"repl external:skip"}} {
            test "XNACK NACKed entries survive full sync (repl-diskless-load $diskless_load)" {
                start_server {tags {"stream"}} {
                    set master [srv 0 client]
                    set master_host [srv 0 host]
                    set master_port [srv 0 port]
                    ...
                    start_server {tags {"stream"}} {
                        ...
                        $replica config set repl-diskless-load $diskless_load
                        $replica replicaof $master_host $master_port
                        wait_for_sync $replica

                        set info [$replica XINFO STREAM mystream FULL]
                        set group [lindex [dict get $info groups] 0]
                        assert_equal [dict get $group nacked-count] 2
                        assert_equal [dict get $group pel-count] 3
