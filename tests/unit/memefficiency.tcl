...
run_solo {defrag} {
    ...
    if {[string match {*jemalloc*} [s mem_allocator]] && [r debug mallctl arenas.page] <= 8192} {
        ...
        test "Active defrag stream nack zone: $type" {
            r flushdb
            r config set hz 100
            r config set activedefrag no
            wait_for_defrag_stop 500 100
            ...
            set n 500000
            set num_streams 20
            set entries_per_stream 100
            set nacked_per_stream 50

            set rd [redis_deferring_client]

            # Create streams with consumer groups and nack zones
            set val [string repeat v 200]
            for {set s 0} {$s < $num_streams} {incr s} {
                set skey "nzstream$s"
                for {set e 0} {$e < $entries_per_stream} {incr e} {
                    $rd xadd $skey * field $val
                }
                for {set e 0} {$e < $entries_per_stream} {incr e} { $rd read }

                r xgroup create $skey grp 0
                r xreadgroup GROUP grp c1 COUNT $entries_per_stream STREAMS $skey >

                set ids {}
                set info_full [r xinfo stream $skey full count 0]
                set group [lindex [dict get $info_full groups] 0]
                set pending [dict get $group pending]
                for {set e 0} {$e < $nacked_per_stream} {incr e} {
                    lappend ids [lindex $pending $e 0]
                }
                r xnack $skey grp FAIL IDS $nacked_per_stream {*}$ids
            }

            # Snapshot nack zone state before defrag
            set pre_defrag_state {}
            for {set s 0} {$s < $num_streams} {incr s} {
                set skey "nzstream$s"
                set info_full [r xinfo stream $skey full count 0]
                set group [lindex [dict get $info_full groups] 0]
                lappend pre_defrag_state [list \
                    [dict get $group nacked-count] \
                    [dict get $group pel-count] \
                    [dict get $group pending]]
            }
            ...
            if {[r config get activedefrag] eq "activedefrag yes"} {
                ...
                # Verify nack zone integrity after defrag
                for {set s 0} {$s < $num_streams} {incr s} {
                    set skey "nzstream$s"
                    set info_full [r xinfo stream $skey full count 0]
                    set group [lindex [dict get $info_full groups] 0]
                    set pre [lindex $pre_defrag_state $s]

                    assert_equal [dict get $group nacked-count] [lindex $pre 0] \
                        "nacked-count mismatch on $skey"
                    assert_equal [dict get $group pel-count] [lindex $pre 1] \
                        "pel-count mismatch on $skey"
                    assert_equal [dict get $group pending] [lindex $pre 2] \
                        "PEL content mismatch on $skey"
                }

                # Walk the entire nack zone linked list by claiming all
                # nacked entries. XREADGROUP CLAIM traverses pel_time_head →
                # pel_next → ... → pel_nack_tail, so this validates every
                # defragged pel_prev/pel_next pointer and pel_nack_tail.
                for {set s 0} {$s < $num_streams} {incr s} {
                    set skey "nzstream$s"
                    after 10
                    set res [r xreadgroup GROUP grp c2 COUNT $nacked_per_stream CLAIM 0 STREAMS $skey >]
                    set entries [lindex [lindex $res 0] 1]
                    assert_equal [llength $entries] $nacked_per_stream \
                        "not all nack zone entries claimed on $skey"
                }
            }
        }
