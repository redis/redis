...
run_solo {defrag} {
    ...
    if {[string match {*jemalloc*} [s mem_allocator]] && [r debug mallctl arenas.page] <= 8192} {
        ...
        test "Active defrag pubsub provenance survives defrag: $type" {
            # A re-authed client's subscriptions carry a provenance stamp (the
            # originating user*) in the dict value slot; active defrag relocates
            # the channel-name objects (dict keys) but must never touch that
            # value slot. After a full defrag cycle we assert both halves:
            #   (a) messages still deliver (channel-name key correctly rewritten)
            #   (b) revoking the ORIGINATING user's channels still kills the
            #       client (provenance value intact), even though its current
            #       user is now 'default'.
            r flushdb
            r config set hz 100
            r config set activedefrag no
            ...
            # Authenticate the subscriber as provuser (RESP3 so it can re-auth
            # while subscribed) and subscribe to n channels, interleaving equally
            # sized filler keys so that deleting them later fragments memory.
            $rd_pubsub HELLO 3 AUTH provuser provuser
            $rd_pubsub read
            for {set j 0} {$j < $n} {incr j} {
                set channel_name "$dummy_channel[format "%06d" $j]"
                $rd_pubsub subscribe $channel_name
            ...
            # Re-auth to default: every still-NULL subscription is stamped with
            # provuser (the identity being left), freezing provenance before
            # defrag runs.
            $rd_pubsub AUTH default ""
            $rd_pubsub read
            $rd_pubsub CLIENT SETNAME prov-defrag
            $rd_pubsub read

            after 120 ;# serverCron only updates the info once in 100ms
            assert_lessthan [s allocator_frag_ratio] 1.1

            # Delete all filler keys to create fragmentation. Batch to avoid
            # TCP deadlock.
            set batch_size 1000
            for {set j 0} {$j < $n} {incr j} {
                $rd del k$j
            ...
            # (a) Delivery still works after defrag relocated channel objects.
            # Sample across the range; the revocation scan below independently
            # walks every subscription, so client-side key integrity is fully
            # exercised too.
            for {set j 0} {$j < $n} {incr j [expr {$n / 20}]} {
                set channel "$dummy_channel[format "%06d" $j]"
                assert_equal 1 [r publish $channel "hello"]
                assert_match {*hello*} [$rd_pubsub read]
            }

            # (b) Provenance survived defrag: the client's current user is now
            # 'default', but its subscriptions are owned by provuser. Revoking
            # provuser's channel access must therefore kill it.
            assert_match {*prov-defrag*} [r CLIENT LIST]
            r ACL SETUSER provuser resetchannels
            wait_for_condition 50 100 {
                ![string match {*prov-defrag*} [r CLIENT LIST]]
            } else {
                fail "client with provuser-owned subscriptions was not killed after channel revocation"
            }
            $rd_pubsub close
