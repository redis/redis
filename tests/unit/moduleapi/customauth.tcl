...
start_server {tags {"modules"}} {
    ...
    test {test custom AUTH for non existing / disabled users} {
        ...
        assert_equal {OK} [r testmoduleone.rm_unregister_auth_cbs]
    }
    ...
    test {test AUTH after registering multiple custom auth callbacks} {
        ...
        # Register multiple callbacks from the same module.
        assert_equal {OK} [r testmoduleone.rm_register_auth_cb]
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]
        assert_equal {OK} [r testmoduleone.rm_register_auth_cb]
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]

        # Register a custom auth callback from the second module.
        ...
        assert_equal {OK} [r testmoduleone.rm_unregister_auth_cbs]
        assert_equal {OK} [r testmoduletwo.rm_unregister_auth_cbs]
    }
    ...
    test {custom auth during blocking custom auth} {
        ...
        # Register a blocking auth cb from moduleone and non blocking cbs from moduleone & moduletwo.
        assert_equal {OK} [r testmoduleone.rm_register_auth_cb]
        assert_equal {OK} [r testmoduletwo.rm_register_auth_cb]
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]
    ...
    test {custom auth inside MULTI EXEC} {
        ...
        # Register a blocking auth cb and a non blocking cb from moduleone.
        assert_equal {OK} [r testmoduleone.rm_register_auth_cb]
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]
    ...
    test {Disabling Redis User during blocking custom auth} {
        ...
        # Register a custom auth callback (that uses a blocking implementation).
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]
    ...
    test {test RM_UnregisterCustomAuthCallbacks Module API} {
        r config resetstat
        set rd [redis_deferring_client]
        r acl setuser foo >pwd on ~* &* +@all

        # Register a blocking auth cb from moduleone and a non blocking cb from moduletwo.
        assert_equal {OK} [r testmoduleone.rm_register_blocking_auth_cb]
        assert_equal {OK} [r testmoduletwo.rm_register_auth_cb]
        ...
        # Attempts to unregister custom auth cbs during blocking custom auth should fail.
        catch {r testmoduleone.rm_unregister_auth_cbs} e
        assert_match {*cannot unregister cbs as custom auth is in progress*} $e
        catch {r testmoduletwo.rm_unregister_auth_cbs} e
        assert_match {*cannot unregister cbs as custom auth is in progress*} $e
        ...
        # Validate that unregistering custom auth cbs succeeds since blocking custom auth is done.
        assert_equal {OK} [r testmoduleone.rm_unregister_auth_cbs]
        assert_equal {OK} [r testmoduletwo.rm_unregister_auth_cbs]

        # Validate that unregistering auth cbs when none exist throws an err.
        catch {r testmoduleone.rm_unregister_auth_cbs} e
        assert_match {*no custom auth cbs registered by this module*} $e
        catch {r testmoduletwo.rm_unregister_auth_cbs} e
        assert_match {*no custom auth cbs registered by this module*} $e
