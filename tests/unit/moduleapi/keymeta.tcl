...
start_server {tags {"modules" "external:skip" "cluster:skip"} overrides {enable-debug-command yes}} {
    ...
    test {KEYMETA - bitmap auto-conversion preserves metadata, expiration, and NO-EVICT} {
        assert_equal 7 $maxClasses
        r config set bitmap-default-roaring no
        r set bitmap:meta:setbit [binary format H* 80]
        r bless set bitmap:meta:setbit no-evict
        setupKeyMeta bitmap:meta:setbit $maxClasses 1 0
        set setbit_expire [r pexpiretime bitmap:meta:setbit]

        r set bitmap:meta:bitfield [binary format H* 80]
        r bless set bitmap:meta:bitfield no-evict
        setupKeyMeta bitmap:meta:bitfield $maxClasses 1 0
