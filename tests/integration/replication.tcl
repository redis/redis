...
# Test for issue #14838: prefetchCommands() crash with incomplete pending commands during replication
# This test verifies that prefetchCommands() handles NULL argv and keys safely when external
# modules send commands while the replica is loading the RDB snapshot.
# The test enables IO threads and diskless sync to exercise the memory prefetch code path.
start_server {tags {"repl external:skip"}} {
    set master [srv 0 client]
    set master_host [srv 0 host]
    set master_port [srv 0 port]
    ...
    start_server {overrides {io-threads 2}} {
        ...
        test {prefetchCommands handles NULL argv and keys during RDB replication with IO threads} {
            ...
            if {[lindex $io_threads 1] > 1} {
                ...
                # Now test the NULL argv scenario by simulating IO thread cleanup
                # This creates a command with partial argv cleanup
                $master config set repl-diskless-sync yes
                $master config set repl-diskless-sync-delay 0
                ...
                wait_for_condition 100 1000 {
                    ...
                    fail "Replica did not complete synchronization for NULL argv test"
                }

                # Verify data was replicated in the NULL argv scenario
                for {set i 0} {$i < 100} {incr i} {
                    assert_equal "value:$i" [$slave get key:$i]
                }
                assert_equal "testvalue" [$slave get testkey]
            } else {
