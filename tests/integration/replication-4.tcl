...
start_server {tags {"repl external:skip"}} {
    start_server {} {
        ...
        test {Data corruption of replication forces full sync} {      
            ...
            assert_equal [count_log_message 0 "Forcing full sync with master to prevent data corruption."] 1
        ...
        test {Data corruption is allowed on writable replicas though} {            
            $replica config set replica-read-only no
            $replica set number foo
            $master incrby number 1

            assert_equal [$master get number] 1
            assert_equal [$replica get number] foo
        }
    }
}
