...
test {corrupt payload: stream with NACK shared between two consumers} {
    start_server [list overrides [list loglevel verbose use-exit-on-panic yes crash-memcheck-enabled no]] {
        r debug set-skip-checksum-validation 1
        # Stream with one entry (1-0), one consumer group (mygroup), two
        # consumers.  consumerB's PEL references the same entry (1-0)
        # that consumerA already owns, making both consumers share the
        # same NACK — an invariant that must never hold.  Without the
        # server-side check this leads to a use-after-free.
