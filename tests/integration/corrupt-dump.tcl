...
test {corrupt payload: hash listpackex TTL of all fields are zero} {
    start_server [list overrides [list loglevel verbose use-exit-on-panic yes crash-memcheck-enabled no] ] {
        r config set sanitize-dump-payload yes
        catch {
            r restore key 0 "\x17\x25\x25\x00\x00\x00\x09\x00\x82\x66\x33\x03\x82\x76\x33\x03\x00\x01\x82\x66\x32\x03\x82\x76\x32\x03\x00\x01\x82\x66\x31\x03\x82\x76\x31\x03\x00\x01\xff\x0c\x00\x84\x5f\xc8\x07\xc7\x40\x3a\x56" replace
        } err
        assert_match "*Bad data format*" $err
        r ping
    }
