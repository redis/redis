...
static int blocked_client_timeout_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    REDISMODULE_NOT_USED(argv);
    REDISMODULE_NOT_USED(argc);
...
int unblock_by_timer(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    RedisModuleBlockedClient *bc = RedisModule_BlockClient(ctx, NULL, blocked_client_timeout_callback, NULL, timeout);
