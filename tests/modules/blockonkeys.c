...
int bpop2_reply_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    RedisModule_Assert(get_fsl(ctx, argv[1], REDISMODULE_READ, 0, &fsl, 0));
    RedisModule_Assert(fsl && fsl->length >= 2);
...
int bpop2_is_key_ready(RedisModuleCtx *ctx) {
    RedisModuleString *keyname = RedisModule_GetBlockedOnKeysKeyname(ctx);

    fsl_t *fsl;
    if (!get_fsl(ctx, keyname, REDISMODULE_READ, 0, &fsl, 0))
        return 0;

    return fsl && fsl->length >= 2;
}
...
int fsl_bpop2(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (!fsl || fsl->length < 2) {
        ...
        RedisModule_BlockClientOnKeys(ctx, bpop2_reply_callback, bpop2_timeout_callback,
                                      timeout, bpop2_is_key_ready, &argv[1], 1, NULL, NULL);
    } else {
        RedisModule_ReplyWithArray(ctx, 2);
        RedisModule_ReplyWithLongLong(ctx, fsl->list[--fsl->length]);
        RedisModule_ReplyWithLongLong(ctx, fsl->list[--fsl->length]);
...
int bpopgt_reply_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    fsl_t *fsl;
    long long gt = (long long)RedisModule_GetBlockedOnKeysMetadata(ctx);

    RedisModule_Assert(argc == 4);
    RedisModule_Assert(get_fsl(ctx, argv[1], REDISMODULE_READ, 0, &fsl, 0));
    RedisModule_Assert(fsl && fsl->list[fsl->length-1] > gt);
...
int bpopgt_is_key_ready(RedisModuleCtx *ctx) {
    RedisModuleString *keyname = RedisModule_GetBlockedOnKeysKeyname(ctx);
    long long gt = (long long)RedisModule_GetBlockedOnKeysMetadata(ctx);
    ...
    return fsl && fsl->list[fsl->length-1] > gt;
}
...
int fsl_bpopgt(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (!fsl || fsl->list[fsl->length-1] <= gt) {
        ...
        RedisModule_BlockClientOnKeys(ctx, bpopgt_reply_callback, bpopgt_timeout_callback,
                                      timeout, bpopgt_is_key_ready, &argv[1], 1, (void*)gt,
