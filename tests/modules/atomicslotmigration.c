...
/* Records whether the REDISMODULE_CTX_FLAGS_SLOT_MIGRATION_IMPORT flag was
 * set the last time keylessCmd ran. -1 means the command has not run yet. */
...
int keylessCmd(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    lastCmdFromAsm = !!(RedisModule_GetContextFlags(ctx) & REDISMODULE_CTX_FLAGS_SLOT_MIGRATION_IMPORT);
