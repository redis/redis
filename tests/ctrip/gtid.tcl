...
start_server {tags {"gtid"} overrides {gtid-enabled yes}} {
    test {COMMANDS} {
        test {GTID SET} {
            r gtid A:1 set x foobar
            r get x 
        } {foobar}

        test {GTID AND COMMENT SET} {
            r gtid A:2 {/*comment*/} set x1 foobar 
            r get x1 
        } {foobar}

        test {GTID REPATE SET} {
            catch {r gtid A:1 set x foobar} error
            assert_match $error "ERR gtid command is executed, `A:1`, `set`,"
        } 
        test {SET} {
            r set y foobar
            r get y 
        ...
        test {MULTI} {
            ...
            r multi 
            r set z foobar1 
            catch {r gtid A:3 exec} error 
            assert_equal $error "ERR gtid command is executed, `A:3`, `exec`,"
