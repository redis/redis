...
Note that the Source may be to local Redis exe files, or may reference a GitHub zip file. For example: -Source https://github.com/MSOpenTech/redis/archive/2.4.zip will download the zip file for the latest code for branch 2.4.
...
After downloading files, you may have to unblock the DLLs to enable them to be used. Please follow instructions at http://msdn.microsoft.com/en-us/library/ee890038(VS.100).aspx
...
To enable RDP to the Azure roles, you need to define the certificates that you will use for RDP to Azure in the cscfg file. To define the certificate, add an XML element under each Role that specifies the thumbprint for the certificate you are using. For example:
...
Acknowledgements
----------------
The Inst4WA binaries were obtained from https://github.com/MSOpenTech/inst4wa.
